...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default="bbox+segm+proposal",
        help='evaluation metrics, which depends on the dataset, e.g., "bbox",'
        ' "segm", "proposal" for COCO, and "mAP", "recall" for PASCAL VOC',
    )
    parser.add_argument(
        "--show", action="store_true", default=True, help="show results"
    )
    parser.add_argument(
...
def main():
    ...
    # show_dir을 자동으로 남기기 위해
    # checkpoint 파일이 있는 디렉토리에 checkpoint이름(latest) 폴더 내 show_dir 폴더 내에 남기기
    checkpoint_path = Path(args.checkpoint)
    show_dir_path = checkpoint_path.parent / checkpoint_path.stem / "show_dir"
    show_dir_path.mkdir(exist_ok=True, parents=True)
    # print(f"show_dir_path: {repr(show_dir_path)} {repr(str(show_dir_path))}")
    # print(f"args.show_dir: {repr(args.show_dir)}")
    ...
    create_submission_file(cfg)


def create_submission_file(cfg: Config) -> None:
